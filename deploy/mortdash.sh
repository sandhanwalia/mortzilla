...
chmod +x deploy/mortdash.sh

# Define your ECR repository and region
ECR_REPO="046267985816.dkr.ecr.us-west-1.amazonaws.com/mortdash"
REGION="us-west-1"

# Check if a version was provided as an argument
if [ -z "$1" ]; then
  echo "Error: No version number provided."
  echo "Usage: ./deploy/mortdash.sh <version>"
  exit 1
fi
...
# Build Docker image
docker build -t mortdash .

# Tag Docker image with specified version
docker tag mortdash:latest $ECR_REPO:$VERSION
